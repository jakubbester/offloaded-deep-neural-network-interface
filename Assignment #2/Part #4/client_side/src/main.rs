...
pub fn display() {
	...
	let file1 = File::options().write(true).read(true).open("/dev/video0").unwrap();
	let fd1 = file1.as_raw_fd();

    // OPEN DEVICE FILE /dev/RustCamera AND GET FILE DESCRIPTOR
    
    let file2 = File::options().write(true).read(true).open("/dev/RustCamera").unwrap();
	...
	match unsafe { vidioc_querycap(fd1, &mut capability as *mut v4l2_capability) } {
		Ok(_) => {
	...
	match unsafe { vidioc_g_fmt(fd1, &mut format as *mut v4l2_format) } {
		Ok(_) => {
	...
	match unsafe { vidioc_s_fmt(fd1, &mut format as *mut v4l2_format) } {
		Ok(_) => {
	...
	match unsafe { vidioc_reqbufs(fd1, &mut requestbuffers as *mut v4l2_requestbuffers) } {
		Ok(_) => {
	...
	match unsafe { vidioc_querybuf(fd1, &mut buffer as *mut v4l2_buffer) } {
		Ok(_) => {
	...
	// STARTING POINT FOR CONVERSION                                              HERE -----------
	...
	match unsafe { vidioc_qbuf(fd1, &mut buffer as *mut v4l2_buffer) } {
		Ok(_) => {
	...
	match unsafe { vidioc_streamon(fd1, std::mem::transmute::<&u32, u64>(&buffer.r#type)) } {
		Ok(_) => {
	...
	loop {
		...
		match unsafe { vidioc_dqbuf(fd1, &mut buffer as *mut v4l2_buffer) } {
			Ok(_) => { } Err(e) => {
		...
		if ANNOTATE.load(Ordering::Relaxed) == true {
			...
			stream.write(&raw).expect("Write to stream [FAILED]");
			...
			draw_keypoints(&mut image, &buffer3, 0.25);
		}
	...
	match unsafe { vidioc_streamoff(fd1, std::mem::transmute::<&u32, u64>(&buffer.r#type)) } {
		Ok(_) => {
